...
import { updateEventSettings, uploadTicketBackground } from "@/app/actions/eventSettings";

export function PengaturanClient({ initialSettings }: { initialSettings: EventSettings | null }) {
      ...
      <div className="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <div className="rounded-xl border border-(--color-border) bg-white p-6">
          ...
          >
            ...
            {uploadingImage ? "Mengunggah..." : backgroundUrl ? "Ganti gambar latar" : "Unggah gambar latar"}
          </button>
          <p className="mt-2 text-xs text-(--color-slate)">Format PNG, JPG, atau WEBP. Maksimal 8MB.</p>
        </div>
      </div>
    </div>
  );
