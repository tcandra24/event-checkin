...
export function RsvpReviewModal({ participant, onClose, onReviewed }: { participant: Participant; onClose: () => void; onReviewed: () => void }) {
  const [finalQty, setFinalQty] = useState(participant.rsvp_qty_response ?? participant.qty);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={onClose}>
      <div className="w-full max-w-sm rounded-2xl bg-white shadow-xl" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between border-b border-(--color-border) px-6 py-4">
          <h2 className="font-display text-base font-semibold text-(--color-ink)">Review Konfirmasi Kehadiran</h2>
        ...
        <div className="px-6 py-5">
          <p className="font-display text-lg font-semibold text-(--color-ink)">{participant.name}</p>
          <p className="mt-1 text-sm text-(--color-slate)">
            {participant.family_group} · Kursi/Meja {participant.seat_number}
          </p>

          <div className="mt-4 rounded-lg bg-blue-50 px-4 py-3 text-sm text-blue-800">
            Peserta mengonfirmasi akan hadir bersama <span className="font-semibold">{participant.rsvp_qty_response} orang</span> (kapasitas tiket maksimal {participant.qty} orang).
          </div>

          <div className="mt-4">
            <label className="mb-1.5 block text-sm font-medium text-(--color-ink)">Jumlah final yang disetujui</label>
            <div className="flex items-center gap-3">
              <button type="button" onClick={() => setFinalQty((q) => Math.max(1, q - 1))} className="flex h-10 w-10 items-center justify-center rounded-lg border border-(--color-border) text-(--color-ink) hover:bg-slate-50" aria-label="Kurangi">
              ...
              <input
                ...
                max={participant.qty}
                value={finalQty}
                onChange={(e) => setFinalQty(Math.min(participant.qty, Math.max(1, Number(e.target.value) || 1)))}
          ...
          <div className="mt-6 flex gap-3">
            ...
            <button onClick={handleApprove} disabled={loading !== null} className="flex flex-1 items-center justify-center gap-2 rounded-lg bg-(--color-emerald) px-4 py-2.5 text-sm font-semibold text-white hover:opacity-90 disabled:opacity-60">
